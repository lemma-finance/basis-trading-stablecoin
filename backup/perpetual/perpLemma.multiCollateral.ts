...
describe("perpLemma.multiCollateral", async function () {
  ...
  async function calcLeverage() {
    ...
    if (!positionSize.eq(ZERO)) {
      console.log(
        'depositedCollateral.mul(parseUnits("1", 26))',
        depositedCollateral.mul(parseUnits("1", 26)).toString(),
      );
      console.log(
        "positionSize.mul(usdcPriceInETHFromIndex.toString())",
        positionSize.mul(usdcPriceInETHFromIndex.toString()),
      );
  ...
  describe("PerpLemma tests => Open, Close, fees, settlement", () => {
    ...
    it("#2", async function () {
      ...
      console.log(
        "amt : ",
        collateralAmountForETH.mul(parseEther("1")).div(parseUnits("1", ethCollateralDecimals)).toString(),
      );
    ...
    it("#4", async function () {
      ...
      await expect(perpLemma2.connect(usdLemma).closeWExactCollateral(collateralAmountForETH)).to.emit(
        clearingHouse,
        "PositionChanged",
      );
      await calcLeverage();
