...
describe("mcdexLemma", async function () {
    ...
    before(async function () {
        ...
        mcdexLemma = await upgrades.deployProxy(MCDEXLemma, [trustedForwarder, liquidityPool.address, perpetualIndex, usdLemma.address, reBalancer.address, maxPosition], { initializer: 'initialize' });
    ...
    it("should set addresses correctly", async function () {
        //setUSDLemma
        await expect(mcdexLemma.connect(signer1).setUSDLemma(signer1.address)).to.be.revertedWith("Ownable: caller is not the owner");
        ...
        //setReferrer
        await expect(mcdexLemma.connect(signer1).setReferrer(signer1.address)).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    it("should open position correctly", async function () {
        ...
        const traderInfoBeforeOpen = await getAccountStorage(reader, liquidityPool.address, perpetualIndex, mcdexLemma.address);
        ...
        const collateralToTransfer = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(amount, true);
        ...
        const traderInfoAfterOpen = await getAccountStorage(reader, liquidityPool.address, perpetualIndex, mcdexLemma.address);
    ...
    it("should fail to open when max position is reached", async function () {
        ...
        await collateral.connect(usdLemma).transfer(mcdexLemma.address, collaterAmountInDecimals);
        await expect(mcdexLemma.connect(usdLemma).open(amount.add(1), collateralToTransfer)).to.be.revertedWith("max position reached");
    ...
    it("should close position correctly", async function () {
        ...
        const traderInfoBeforeClose = await getAccountStorage(reader, liquidityPool.address, perpetualIndex, mcdexLemma.address);
        expect(traderInfoBeforeClose.positionAmount.toString()).to.equal("1000");

        const collateralToGetBack = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(amount, false);
        await mcdexLemma.connect(usdLemma).close(amount, collateralToGetBack);
        const traderInfoAfterClose = await getAccountStorage(reader, liquidityPool.address, perpetualIndex, mcdexLemma.address);
    ...
    it("should return collateral amount required correctly", async function () {
        ...
        const collateralToTransferForOpening = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(utils.parseEther(amount), true);
        ...
        const collateralToTransferForClosing = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(utils.parseEther(amount), false);
    ...
    describe("should calculate fundingPNL correctly", async function () {
        ...
        afterEach(async function () {
            ...
            for (let i = 0; i <= 2; i++) {
                ...
                const collateralToTransfer = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(utils.parseEther(amount), false);
                await collateral.connect(usdLemma).transfer(mcdexLemma.address, collateralToTransfer);
    ...
    describe("when settled", async function () {
        ...
        it("should not be able to open", async function () {
            const amount = utils.parseEther("1000");
            await expect(mcdexLemma.getCollateralAmountGivenUnderlyingAssetAmount(amount, true)).to.be.revertedWith("cannot open when perpetual has settled");
        });
        ...
        it("should settle + close correctly", async function () {
            ...
            expect((await collateral.balanceOf(mcdexLemma.address)).sub(preBalance)).to.equal(convertToCollateralDecimals(settleableMargin.toString(), collateralDecimals));
            ...
            const collateralBalanceAfter = await collateral.balanceOf(usdLemma.address);
            //usdlemma balance change == settleableMargin
            expect(collateralBalanceAfter.sub(collateralBalanceBefore)).to.be.closeTo(convertToCollateralDecimals(settleableMargin.toString(), collateralDecimals), amt);
        });
        it("should return collateral required correctly", async function () {
            const amount = utils.parseEther("1000");
            //fails if tries to open
            await expect(mcdexLemma.getCollateralAmountGivenUnderlyingAssetAmount(amount, true)).to.be.revertedWith("cannot open when perpetual has settled");
            const collateralRequiredFromContractForAllAmount = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(amount, false);
            ...
            const collateralRequiredForHalfAmount = await mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(amount.div(2), false);
    ...
    describe("re balance", async function () {
        ...
        afterEach(async function () {
            ...
            await mcdexLemma.connect(usdLemma).reBalance(reBalancer.address, await fromBigNumber(amountWithFeesConsidered), ethers.utils.defaultAbiCoder.encode(["int256", "uint256"], [limitPrice, deadline]));
            {
                const liquidityPoolInfo = await getLiquidityPool(reader, liquidityPool.address);
                const traderInfo = await getAccountStorage(reader, liquidityPool.address, perpetualIndex, mcdexLemma.address);
                const account = computeAccount(liquidityPoolInfo, perpetualIndex, traderInfo);
