    ...
    describe("PerpLemma tests => Open, Close, fees, settlement", () => {
        before(async function () {
            ...
            await addLiquidity(
                ...
                parseEther('10000'), parseEther('100'), -887200, 887200
            )
        });
        ...
        it("should set addresses correctly", async function () {
            ...
            await expect(perpLemma.connect(signer1).setReferrerCode(ethers.utils.formatBytes32String("Hello World"))).to.be.revertedWith("Ownable: caller is not the owner");
        ...
        it("should fail to open when max position is reached", async function () {
            ...
            await expect(perpLemma.connect(usdLemma).openWExactCollateral(collateralAmount)).to.be.revertedWith("max position reached");
        })
        ...
        it("should close position correctly", async function () {
            ...
            collateralAmount = collateralAmount.sub(collateralAmount.mul(BigNumber.from('10000')).div(BigNumber.from('1000000')))
            baseAndQuoteValue = await callStaticOpenPosition(clearingHouse, longAddress, baseToken.address, false, true, collateralAmount) // index0: base/usd, index1: quote/eth
            ...
            await expect(perpLemma.connect(usdLemma).openWExactCollateral(collateralAmount))
                .to.emit(clearingHouse, 'PositionChanged').withArgs(
        ...
        describe("OpenPosition tests2", () => {
            ...
            it("openPosition => open position for short and close position for 2 time longs", async () => {
                ...
                // #1    
                baseAndQuoteValue = await callStaticOpenPosition(clearingHouse, longAddress, baseToken.address, true, true, positionSize.div(2))
                collateralAmount = baseAndQuoteValue[1].mul(parseEther('1')).div(parseEther('0.99'))
