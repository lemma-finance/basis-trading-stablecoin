...
describe("mcdexLemma", async function () {
  ...
  it("should set addresses correctly", async function () {
    //setUSDLemma
    await expect(this.mcdexLemma.connect(signer1).setUSDLemma(signer1.address)).to.be.revertedWith(
    ...
    //setReferrer
    await expect(this.mcdexLemma.connect(signer1).setReferrer(signer1.address)).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );
  ...
  it("should deposit keeper reward correctly", async function () {
    const traderInfoBefore = await getAccountStorage(
      reader,
    ...
    const traderInfoAfter = await getAccountStorage(
      reader,
      liquidityPool.address,
      perpetualIndex,
      this.mcdexLemma.address,
  ...
  it("should open position correctly", async function () {
    ...
    const collateralToTransfer = await this.mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(
      amount,
      true,
    );
  ...
  it("should fail to open when max position is reached", async function () {
    ...
    await this.collateral.connect(usdLemma).transfer(this.mcdexLemma.address, collaterAmountInDecimals);
    await expect(this.mcdexLemma.connect(usdLemma).open(amount.add(1), collateralToTransfer)).to.be.revertedWith(
  ...
  it("should close position correctly", async function () {
    ...
    const collateralToGetBack = await this.mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(
      amount,
      false,
    );
  ...
  it("should return collateral amount required correctly", async function () {
    ...
    const collateralToTransferForOpening =
      await this.mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(utils.parseEther(amount), true);
    expect(
      BigNumber.from(
    ...
    const collateralToTransferForClosing =
      await this.mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(utils.parseEther(amount), false);
    expect(
      BigNumber.from(
  ...
  describe("should calculate fundingPNL correctly", async function () {
    ...
    afterEach(async function () {
      ...
      for (let i = 0; i <= 3; i++) {
        ...
        const liquidityPoolInfo = await getLiquidityPool(reader, liquidityPool.address);
        let traderInfo = await getAccountStorage(
          reader,
          liquidityPool.address,
          perpetualIndex,
          this.mcdexLemma.address,
  ...
  describe("when settled", async function () {
    ...
    it("should not be able to open", async function () {
      const amount = utils.parseEther("1000");
      await expect(this.mcdexLemma.getCollateralAmountGivenUnderlyingAssetAmount(amount, true)).to.be.revertedWith(
    ...
    it("should close correctly", async function () {
      ...
      let { settleableMargin } = await liquidityPool.getMarginAccount(perpetualIndex, this.mcdexLemma.address);
    ...
    it("should settle + close correctly", async function () {
      ...
      expect((await this.collateral.balanceOf(this.mcdexLemma.address)).sub(preBalance)).to.equal(
        convertToCollateralDecimals(settleableMargin.toString(), this.collateralDecimals),
      );
      ...
      // await printTx(tx.hash);
      expect(await this.collateral.balanceOf(this.mcdexLemma.address)).to.be.closeTo(
        preBalance,
        utils.parseUnits("0.05", this.collateralDecimals),
      );
      const collateralBalanceAfter = await this.collateral.balanceOf(usdLemma.address);
      //usdlemma balance change == settleableMargin
    ...
    it("should return collateral required correctly", async function () {
      ...
      const account = await liquidityPool.getMarginAccount(perpetualIndex, this.mcdexLemma.address);
      expect(collateralRequiredFromContractForAllAmount).to.be.closeTo(
        account.settleableMargin,
        utils.parseUnits("0.05", 18),
      );

      const collateralRequiredForHalfAmount =
        await this.mcdexLemma.callStatic.getCollateralAmountGivenUnderlyingAssetAmount(amount.div(2), false);
      expect(collateralRequiredForHalfAmount).to.be.closeTo(
        account.settleableMargin.div(2),
        utils.parseUnits("0.05", 18),
      );
    });
  });
