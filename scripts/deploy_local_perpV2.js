...
const { displayNicely, tokenTransfers, loadPerpLushanInfo, loadPerpLushanInfoMainnet, toBigNumber, fromBigNumber, snapshot, revertToSnapshot } = require("../test/utils");
...
async function main() {
    ...
    let [defaultSigner, reBalancer, hasWETH, keeperGasReward, signer1, signer2, longAddress, lemmaTreasury] = await ethers.getSigners();
    ...
    const usdLemma = await upgrades.deployProxy(USDLemma, [AddressZero, collateralAddress, perpLemma.address], { initializer: 'initialize' });
